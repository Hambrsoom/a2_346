...
public class BlockManager
{
	 ...
	 * For synchronization
	 */
	//it starts with -9 since we have 10 threads in total
	...
	static class AcquireBlock extends BaseThread
	{
		...
		public void run()
		{
			...
			catch(Exception e)
			{
				...
				//we increase by one the value of the semaphore until we reach to 0 --> to start running phase 2
				s1.V();
				System.out.println("\n\n Thread " + this.iTID + " has finished phase I \n\n");
			}
			s2.P(); // From this point, only one thread can enter phase 2 at a time. 
			while (!turnTestAndSet()) {
// These two lines are to give other threads a chance to check their turn.
				s2.V(); 
				s2.P(); 			
			}
			s1.P();
			if (value ==1) {
				System.out.println("\n\nAll threads are done from Phase 1\n\n");
				value--;
			}
			phase2();
			s1.V();
